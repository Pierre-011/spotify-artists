...
function displayCurrentDate() {
    ...
    if (currentDate) currentDate.textContent = formatDate(today);
    ...
    if (releaseDescription) {
        releaseDescription.textContent = `Sorties prévues le ${formatReadableDate(today)}`;
    }
}
...
async function loadJSON(filePath) {
    ...
    if (!response.ok) {
        throw new Error(`Erreur HTTP ${response.status} pour ${url.pathname}`);
    }
...
function parseArtists(data) {
    ...
    if (data.artists && typeof data.artists === "object") {
        return Object.values(data.artists).filter(a => a && typeof a === "object");
    }
    if (typeof data === "object" && !data.id && !data.name) {
        return Object.values(data).filter(a => a && typeof a === "object");
    }
...
function parseReleases(data) {
    if (!data) return [];
    if (Array.isArray(data)) return data.filter(Boolean);
    if (Array.isArray(data.releases)) return data.releases.filter(Boolean);
    if (Array.isArray(data.tracks)) return data.tracks.filter(Boolean);
    if (Array.isArray(data.albums)) return data.albums.filter(Boolean);
    if (data.releases && typeof data.releases === "object") {
        return Object.values(data.releases).flat().filter(Boolean);
    }
    ...
    if (typeof data === "object" && !data.id && !data.name && !data.release_date) {
        return Object.values(data).filter(r => r && typeof r === "object");
    }
    if (data.id || data.name || data.release_date || data.album_name) return [data];
    return [];
}

function getSearchValue(id) {
    const element = $(id);
    return element ? element.value.trim().toLocaleLowerCase("fr-FR") : "";
}
...
function renderReleases() {
    ...
    const releasesToday = releases.filter(release => getReleaseDate(release) === today);
    ...
    if (countElement) {
        countElement.textContent = `${filteredReleases.length}`;
    }
    ...
    container.innerHTML = filteredReleases
        ...
        .map(release => {
            const title = release.name || release.album_name || release.albumName || "Titre inconnu";
            const artist = release.artist_name || release.artistName || "Artiste inconnu";
            const image = release.album_image || release.albumImage || release.image || "";
            ...
            return `
                <article class="release-card">
                    ...
                    <div class="release-information">
                        ...
                        <div class="release-artist">${escapeHTML(artist)}</div>
                        <div class="release-album">${formatDate(today)}</div>
                        ${
                            url !== "#"
                                ? `<a class="spotify-button" href="${escapeHTML(url)}" target="_blank" rel="noopener noreferrer">Écouter sur Spotify</a>`
                                : ""
                        }
                    </div>
                </article>
...
function renderArtists() {
    ...
    let filteredArtists = artists.filter(artist => {
        ...
        return name.toLocaleLowerCase("fr-FR").includes(search);
    });

    if (sort === "followers") {
        filteredArtists.sort((a, b) => Number(b.followers || 0) - Number(a.followers || 0));
    } else if (sort === "popularity") {
        filteredArtists.sort((a, b) => Number(b.popularity || 0) - Number(a.popularity || 0));
    } else {
        filteredArtists.sort((a, b) => {
            const nameA = a.name || a.artist_name || "";
            const nameB = b.name || b.artist_name || "";
            return nameA.localeCompare(nameB, "fr-FR", { sensitivity: "base" });
        });
    }

    const artistCount = $("artist-count");
    if (artistCount) artistCount.textContent = formatNumber(artists.length);

    if (filteredArtists.length === 0) {
        container.innerHTML = `
            <tr>
                <td colspan="6" class="muted">Aucun artiste trouvé.</td>
            </tr>
        `;
        return;
    }
    ...
    container.innerHTML = filteredArtists.map(artist => {
        const name = artist.name || artist.artist_name || "Artiste inconnu";
        const spotifyURL = artist.url || artist.external_urls?.spotify || "";
        ...
        return `
            <tr>
                <td class="artist-name">
                    ${
                        spotifyURL
                            ? `<a class="artist-link" href="${escapeHTML(spotifyURL)}" target="_blank" rel="noopener noreferrer">${escapeHTML(name)}</a>`
                            : escapeHTML(name)
                    }
                </td>
                <td>${formatNumber(artist.followers)}</td>
                <td>${formatNumber(artist.monthly_listeners)}</td>
                <td>${artist.popularity ?? "—"}</td>
                <td class="genres-cell">${escapeHTML(genres || "—")}</td>
                <td>
                    ${
                        spotifyURL
                            ? `<a class="spotify-button" href="${escapeHTML(spotifyURL)}" target="_blank" rel="noopener noreferrer">Spotify</a>`
                            : "—"
                    }
                </td>
            </tr>
...
function renderGenres() {
    ...
    if (genres.length === 0) {
        container.innerHTML = `<div class="empty">Aucun genre disponible.</div>`;
        return;
    }

    container.innerHTML = genres.map(([genre, count]) => `
        <div class="genre-card">
            <div class="genre-count">${formatNumber(count)}</div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (artistSearch) artistSearch.addEventListener("input", renderArtists);

    const artistSort = $("artist-sort");
    if (artistSort) artistSort.addEventListener("change", renderArtists);

    const releaseSearch = $("release-search");
    if (releaseSearch) releaseSearch.addEventListener("input", renderReleases);
